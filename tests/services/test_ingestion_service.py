...
def mock_splitter(mocker):
    ...
    mock = mocker.patch("src.services.ingestion.ingestion_service.RecursiveCharacterTextSplitter")
    instance = mock.return_value
...
async def test_ingest_data_success(mock_loader, mock_splitter, mock_vector_store):
    ...
    chunk_count = await service.ingest_data(
        file_path="fake/path/doc.pdf", user_id="user_123", extra_metadata={"status": "uploaded"}
    )
