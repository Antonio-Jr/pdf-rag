...
async def test_chatbot_node_success(mock_get_model, mocker):
    ...
    mock_prompt.get_prompt.return_value.format_messages.return_value = [{"role": "system", "content": "You are a helpful AI"}]
    ...
    assert isinstance(new_state.messages[0], AIMessage) or new_state.messages[0].content == "I am ready to help."
...
async def test_chatbot_node_with_tool_calls(mock_get_model, mocker):
    ...
    mock_prompt = mocker.patch("src.services.nodes.chabot_node.prompt_registry")
    mock_prompt.get_prompt.return_value.format_messages.return_value = [{"role": "sys", "content": "AI"}]
    ...
    mock_response = MagicMock(content="", tool_calls=[{"name": "extraction_tool", "args": {"points": ["a"]}}])
    mock_model_instance.ainvoke = mocker.AsyncMock(return_value=mock_response)
    ...
    logger_instance.info.assert_called_with(f"🛠️ [TOOL CALL]: extraction_tool with args: {{'points': ['a']}}")
